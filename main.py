"""
FreeUniHomos — серверна частина (backend), версія 5
=======================================================

Нове порівняно з v4:
- Online / last_seen для користувачів (активні зверху в чатах)
- Войс-кімнати: список доступних, хто говорить, max_members
- Ефемерні повідомлення в войсі (емодзі / текст ≤100 символів) — авто-видалення через 50 с
- Presence heartbeat
"""
...
def health_check():
    ensure_world_chat()
    return {"status": "ok", "service": "FreeUniHomos API", "version": 5}
...
def create_entity(payload: CreateEntityRequest):
    ...
    full_handle = payload.kind + suffix
...
# ---------------------------------------------------------------------------
# Сторіс (фото + текст до 300 символів), реакція (одна на людину), коментарі
# ---------------------------------------------------------------------------

@app.post("/profile/settings/update")
def update_post_settings(payload: UpdatePostSettingsRequest):
    handle = clean_handle(payload.handle)
    if not users.find_one({"handle": handle}):
        raise HTTPException(status_code=404, detail="User not found")
    users.update_one({"handle": handle}, {"$set": {
        "allow_view_posts": payload.allow_view_posts,
        "allow_react_posts": payload.allow_react_posts,
        "allow_comment_posts": payload.allow_comment_posts,
    }})
    return public_user(users.find_one({"handle": handle}))
...
def create_story(payload: CreateStoryRequest):
    ...
    if not payload.photo and not text:
        raise HTTPException(status_code=400, detail="Story needs a photo or text")

    story_id = str(uuid.uuid4())
    stories.insert_one({
        "story_id": story_id, "owner_handle": owner,
        "photo": payload.photo, "text": text,
...
def list_stories(handle: str, viewer: str = ""):
    owner = clean_handle(handle)
    viewer = clean_handle(viewer) if viewer else ""
    owner_user = users.find_one({"handle": owner})
    if not owner_user:
        raise HTTPException(status_code=404, detail="User not found")
    if viewer != owner and not owner_user.get("allow_view_posts", True):
        return []

    docs = list(stories.find({"owner_handle": owner}, {"_id": 0}).sort("created_at", -1))
    for d in docs:
        reactions = list(story_reactions.find({"story_id": d["story_id"]}, {"_id": 0}))
        d["reaction_count"] = len(reactions)
        d["my_reaction"] = any(r["handle"] == viewer for r in reactions) if viewer else False
        d["comment_count"] = story_comments.count_documents({"story_id": d["story_id"]})
    return docs


@app.post("/stories/react")
def react_story(payload: StoryReactRequest):
    story_id = payload.story_id
    handle = clean_handle(payload.handle)
    story = stories.find_one({"story_id": story_id})
    if not story:
        raise HTTPException(status_code=404, detail="Story not found")
    owner_user = users.find_one({"handle": story["owner_handle"]})
    if owner_user and not owner_user.get("allow_react_posts", True) and story["owner_handle"] != handle:
        raise HTTPException(status_code=403, detail="Reactions are turned off for this person's posts")

    existing = story_reactions.find_one({"story_id": story_id, "handle": handle})
    if existing:
        story_reactions.delete_one({"story_id": story_id, "handle": handle})
        return {"reacted": False}
    story_reactions.insert_one({"story_id": story_id, "handle": handle, "reaction": "like"})
    return {"reacted": True}


@app.get("/stories/comments/{story_id}")
def get_story_comments(story_id: str):
    docs = list(story_comments.find({"story_id": story_id}, {"_id": 0}).sort("created_at", 1))
    senders = user_lookup_map({d["handle"] for d in docs})
    for d in docs:
        info = senders.get(d["handle"], {})
        d["name"] = info.get("name", d["handle"])
        d["avatar"] = info.get("avatar", "")
    return docs


@app.post("/stories/comment")
def add_story_comment(payload: StoryCommentRequest):
    handle = clean_handle(payload.handle)
    text = payload.text.strip()
    if not text or len(text) > 200:
        raise HTTPException(status_code=400, detail="Comment must be 1-200 characters")
    story = stories.find_one({"story_id": payload.story_id})
    if not story:
        raise HTTPException(status_code=404, detail="Story not found")
    owner_user = users.find_one({"handle": story["owner_handle"]})
    if owner_user and not owner_user.get("allow_comment_posts", True) and story["owner_handle"] != handle:
        raise HTTPException(status_code=403, detail="Comments are turned off for this person's posts")

    comment_id = str(uuid.uuid4())
    story_comments.insert_one({
        "comment_id": comment_id, "story_id": payload.story_id,
        "handle": handle, "text": text, "created_at": time.time(),
    })
    return {"comment_id": comment_id}


@app.post("/stories/comment/delete")
def delete_story_comment(payload: DeleteStoryCommentRequest):
    handle = clean_handle(payload.handle)
    comment = story_comments.find_one({"comment_id": payload.comment_id})
    if not comment:
        raise HTTPException(status_code=404, detail="Comment not found")
    story = stories.find_one({"story_id": comment["story_id"]})
    is_owner_of_story = story and story["owner_handle"] == handle
    if comment["handle"] != handle and not is_owner_of_story:
        raise HTTPException(status_code=403, detail="Not allowed")
    story_comments.delete_one({"comment_id": payload.comment_id})
    return {"status": "ok"}


@app.post("/stories/delete")
def delete_story(payload: DeleteStoryRequest):
    handle = clean_handle(payload.handle)
    story = stories.find_one({"story_id": payload.story_id})
    if not story:
        raise HTTPException(status_code=404, detail="Story not found")
    if story["owner_handle"] != handle:
        raise HTTPException(status_code=403, detail="Not yours")
    stories.delete_one({"story_id": payload.story_id})
    story_reactions.delete_many({"story_id": payload.story_id})
    story_comments.delete_many({"story_id": payload.story_id})
    return {"status": "ok"}
