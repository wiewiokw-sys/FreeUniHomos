"""
FreeUniHomos — серверна частина (backend), версія 3
=======================================================

Нове порівняно з попередньою версією:
- Редагування / видалення власних повідомлень (DM, Збережене, канали/чати/войси)
- Відповідь на повідомлення (reply_to) і Переслати (forwarded_from)
- В повідомленнях каналу/чату/войсу тепер повертається ім'я + аватар відправника
- Емодзі-тег профілю (tag_emoji) — зберігається і повертається всюди, де є профіль
- /users/by-id/{user_id} — дозволяє телефону "звірити" свій акаунт із сервером
  (виправляє баг, коли хендл змінили на одному пристрої, а другий про це не знав)
- Видалення акаунта
- Канали/чати/войси тепер створюються з хендлом, що вже містить префікс типу
  (наприклад @chat123, @channel123, @voice123) — így легко шукати і не плутати типи
"""
